...
class LIFNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, tau_mem=20e-3, tau_syn=10e-3, dt=1e-3, beta=20.0,
                 reset=False, simulation_timesteps=10, hebbian=True, eta=1e-4, use_bias=False):
    ...
    def get_spikes(self):
        spikes = []
        for i in range(len(self.layers)):
            # The layer should have a get_spikes method for this to work
            if hasattr(self.layers[i], 'get_spikes'):
                layer_spikes = self.layers[i].get_spikes()
                spikes.append(layer_spikes)
        return spikes
...
class HebbianSimonAgent:
    ...
    def get_spikes(self):
        return self.online_network.get_spikes()